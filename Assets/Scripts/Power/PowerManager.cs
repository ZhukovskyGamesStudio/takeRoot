...
public class PowerManager : MonoBehaviour {
    private List<PowerConsumer> _powerConsumers = new();
    private List<PowerProvider> _generators = new();
    private Dictionary<Vector2Int, Wire> _wires = new();

    private HashSet<Vector2Int> _checkedWires = new();

    public List<Sprite> WireDirectionSprites = new();
    ...
    private void Update() {
        ...
        if (Input.GetKeyDown(KeyCode.R)) {
            Wire wire = _wires[Camera.main.ScreenToWorldPoint(Input.mousePosition).ToVector2Int()];
    ...
    public void OnWireDestroyed(Wire wire) {
        Vector2Int pos = wire.transform.position.ToVector2Int();
        if (!_wires.ContainsKey(pos)) {
            return;
        }
    ...
    private void UpdateWireConnection() {
        ...
        foreach (PowerProvider biogenerator in _generators) {
            byte connected = FloodFill(biogenerator.PowerSocketPosition, biogenerator);
    ...
    private void UpdatePowerConsumersConnection() {
        foreach (PowerConsumer powerConsumer in _powerConsumers) {
            if (_wires.TryGetValue(powerConsumer.PowerSocketPosition, out Wire wire)) {
    ...
    private byte FloodFillNotConnected(Vector2Int pos) {
        if (!_wires.TryGetValue(pos, out Wire wire)) {
            return 0;
        }

        if (!_checkedWires.Add(pos)) {
            return 1;
        }
