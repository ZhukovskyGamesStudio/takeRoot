...
public class IterativeDeepeningAStar : IPathfindService{
	...
	public List<Vector2> FindPath(Vector2 start, Vector2 end) {
		...
		var startInt = new int3((int)start.x, (int)start.y, 0); //TODO: change when layer added
		var goalInt = new int3((int)end.x, (int)end.y, 0); //TODO: change when layer added
		var path = FindPath(_graph, startInt, goalInt);
	...
	public List<int3> FindPath(SimpleGraph graph, int3 startPos, int3 endPos)
	{
		...
		var path = new List<SimpleNode>((int)threshold) { start };
		var pathEdges = new List<SimpleEdge>();
		var visited = new HashSet<SimpleNode>((int)threshold);
	...
	private float Search(
		...
		float g,
		float threshold,
		List<SimpleNode> path,
		List<SimpleEdge> pathEdges,
