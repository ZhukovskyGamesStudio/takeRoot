...
public class BuildingPlan : ECSEntity
{
    ...
    [SerializeField] private int _buildPoints;
    [SerializeField]private List<ResourceData> _requiredResources;
    [SerializeField]private List<ResourceData> _currentResources;

    [SerializeField]private List<CommandData> _activeGatherCommands;
    [SerializeField]private Dictionary<ResourceType, int> _reservedResourceAmount = new Dictionary<ResourceType, int>();
    ...
    {
        return _requiredResources.Find(r => r.ResourceType == type).Amount ==
    ...
    public List<ResourceView> FindFitResourcesOnGround(ResourceType type)
    {
        ...
        return resourcesOnGround.Where(r => r.ResourceType == type && r.Amount != 0 && r.GetEcsComponent<Interactable>().CommandToExecute == null).ToList();
    ...
    private List<Storagable> FindFitStorages(ResourceType type)
    {
        ...
        return storages.Where(r =>
            r.Resource.ResourceType == type && r.Resource.Amount != 0 &&
            r.GetComponent<Interactable>().CommandToExecute == null).ToList();
    }
