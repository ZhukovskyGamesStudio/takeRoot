...
public class SettlerEditor : Editor
{
    ...
    private Dictionary<BTNode, BTNodeDebugInfo> _nodeDebugInfos = new Dictionary<BTNode, BTNodeDebugInfo>();
    ...
    private void FindLastEvaluatedNode(BTNode node, FieldInfo stateField)
    {
        ...
        var state = (BTNodeState)stateField.GetValue(node);
        ...
        {
            var childrenField = typeof(Selector).GetField("_children", BindingFlags.NonPublic | BindingFlags.Instance);
            var children = (List<BTNode>)childrenField?.GetValue(selector);
            if (children != null)
            {
        ...
        {
            var childrenField = typeof(Sequence).GetField("_children", BindingFlags.NonPublic | BindingFlags.Instance);
            var children = (List<BTNode>)childrenField?.GetValue(sequence);
            if (children != null)
            {
                foreach (var child in children)
                {
    ...
    {
        if (node == null || _richTextStyle == null) return;
        
        if (!_nodeDebugInfos.TryGetValue(node, out var debugInfo))
        {
        ...
        string indent = new string(' ', depth * 15);
        ...
        {
            debugInfo.IsExpanded = EditorGUILayout.Foldout(
    ...
    private bool IsParentOfLastEvaluated(BTNode potentialParent, BTNode node)
    {
        ...
        var children = GetChildren(potentialParent);
        foreach (var child in children)
        {
    ...
    private List<BTNode> GetChildren(BTNode node)
    {
        ...
        if (node is Selector selector)
        {
            var field = typeof(Selector).GetField("_children", BindingFlags.NonPublic | BindingFlags.Instance);
            if (field != null)
            {
        ...
        {
            var field = typeof(Sequence).GetField("_children", BindingFlags.NonPublic | BindingFlags.Instance);
            if (field != null)
            {
