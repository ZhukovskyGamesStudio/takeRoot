...
public class EntryPoint_Video2 : MonoBehaviour, ICoroutineRunner {
    ...
    private void Awake() {
        ...
        UpdateService updateService = GetComponent<UpdateService>();
    ...
    private IEnumerator PlayVideo() {
        ...
        yield return StartCoroutine(PlayEmotionAndWait(0, tosterEmotes.playTime,
        ...
        CreateMoveCommand(moveToCornerOfTheRoad, chamomile).onComplete += () => {
        ...
        CreateMoveCommand(moveToRoad2, chamomile).onComplete += () => CreateMoveCommand(moveToGenerator, chamomile).onComplete += () => {
    ...
    private BaseCommand CreateMoveCommand(Transform pos, Worker worker) {
        return new MoveToCommand(_id++, pos.position, _services.Single<ICommandService>(), _services.Single<IUpdateService>(), worker);
    }

    private BaseCommand CreateSearchCommand(CommandTarget commandTarget, Worker worker) {
        return new SearchCommand(_id++, commandTarget, _services.Single<ICommandService>(), _services.Single<IUpdateService>(), worker);
    }

    private BaseCommand CreateSearchCommand(CommandTarget commandTarget) {
        return new SearchCommand(_id++, commandTarget, _services.Single<ICommandService>(), _services.Single<IUpdateService>());
    }
