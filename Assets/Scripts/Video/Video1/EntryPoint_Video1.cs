...
public class EntryPoint_Video1 : MonoBehaviour, ICoroutineRunner {
    ...
    [FormerlySerializedAs("RemboEmotionPlayer")]
    public EmotionPlayer remboEmotionPlayer;
    ...
    private BaseCommand CreateMoveCommand(Transform pos, int id) {
        return new MoveToCommand(id, pos.position, _services.Single<ICommandService>(), _services.Single<IUpdateService>(), mainWorker);
    }

    private BaseCommand CreateWaterCommand(CommandTarget target, int id) {
        return new WaterCommand(id, target, _services.Single<ICommandService>(), _services.Single<IUpdateService>(), mainWorker);
    }
            ...
            CreateMoveCommand(swapPos, 4).onComplete += () => {
                ...
                CreateMoveCommand(unZoomPos2, 5).onComplete += () => {
                    ...
                    CreateMoveCommand(emotePos, 6).onComplete += () => {
                        ...
                        StartCoroutine(PlayEmotionAndWait(0f, 0f, () => remboEmotionPlayer.PlayEmotion(BubbleType.Talk, EmotionType.Like)));
