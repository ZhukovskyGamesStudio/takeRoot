...
public class AStarPathfindingVertical : MonoBehaviour {
    [SerializeField]
    private GameObject pathSquare;

    [SerializeField]
    private int width = 1000, height = 1000, layers = 4;

    //private readonly List<Node> _neighbors = new List<Node>(5);
    private readonly List<Point> _path = new(500);
    ...
    public void Start() {
        _warpManager = FindObjectsByType<WarpManager>(FindObjectsInactive.Exclude, FindObjectsSortMode.None)[0]; //TODO: singleton
    ...
    private void CreateMap() {
        WorldLayer[] worldLayers = FindObjectsByType<WorldLayer>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
        foreach (WorldLayer worldLayer in worldLayers) {
            ...
            foreach (Vector2Int pos in gridable.GetOccupiedLocalPositions()) {
                Point point = new(pos.x, pos.y, worldLayer.Layer);
    ...
    public List<Point> FindPath(Point start, Point end, out bool isPathExist) {
        ...
        Point goal = end;
        ref NodeData startNode = ref _grid[start.X, start.Y, start.Layer];
        startNode.gCost = 0;
        ...
        while (_openQueue.Count > 0) {
            counter++;
            Point current = _openQueue.Dequeue();
            ref NodeData currentNode = ref _grid[current.X, current.Y, current.Layer];
            ...
            for (int i = 0; i < neighborCount; i++) {
                Point neighbor = _neighbors[i];
                ref NodeData neighborNode = ref _grid[neighbor.X, neighbor.Y, neighbor.Layer];
    ...
    private int GetNeighbors(Point p, Point[] output) {
        ...
        if (_warpManager.warps.TryGetValue(p, out Point warpTarget)) {
    ...
    private List<Point> Retrace(Point start, Point end) {
        ...
        while (!current.Equals(start)) {
            path.Add(current);
            ref NodeData node = ref _grid[current.X, current.Y, current.Layer];
...
public class PriorityHeap<T> {
    ...
    public T Dequeue() {
        ...
        (T item, int priority) last = _elements[^1];
        _elements[0] = last;
