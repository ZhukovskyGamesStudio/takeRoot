...
public class Worker : MonoBehaviour {
    ...
    public void MoveTo(Vector2 position) {
        _mover.MoveTo(position); //TODO: change animations play while moving
    }

    public bool HasPath(Vector2 position) {
        return _mover.HasPath(position);
    }

    public bool IsAtPosition(Vector2 position) {
        return _mover.IsAtPosition(position);
    }
    ...
    private void ExecutedWithAnimation<T>(AnimatorState targetState, Action<T> action, T parameter, Action playAnimation, float time)
        where T : CommandTarget {
        if (WorkerAnimator.State != AnimatorState.Idle || Performing) {
            return;
        }
