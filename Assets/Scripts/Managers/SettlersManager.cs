...
public class SettlersManager : MonoBehaviour, IInitableInstance {
    ...
    private HashSet<Settler> _settlers = new();
    private HashSet<SettlerData> _settlersDatas = new();
    ...
    public void DestroySettler(Settler settler) {
        TacticalInteractable interactable = settler.GetEcsComponent<TacticalInteractable>();
    ...
    public void CreateCombinedSettlerAt(Vector2Int position) {
        Settler instance = Instantiate(_combinedSettlerPrefab, new Vector3(position.x, position.y), Quaternion.identity) as Settler;
        Settlers.Add(instance);
        _settlersDatas.Add(instance.SettlerData);
    }

    public void SpawnSettlerAt(Race race, Vector2Int position) {
        Settler settler = Instantiate(race == Race.Plants ? _chamomilePrefab : _lampPrefab, new Vector3(position.x, position.y),
