...
public class SettlersManager : MonoBehaviour, IInitableInstance {
    ...
    public List<Settler> MySettlers => FindObjectsByType<Settler>(FindObjectsInactive.Exclude, FindObjectsSortMode.None)
        .Where(s => s.Race == Race.Both || s.Race == Core.Instance.MyRace()).ToList();
