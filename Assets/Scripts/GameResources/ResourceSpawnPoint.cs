...
public class ResourceSpawnPoint : MonoBehaviour {
    ...
    [SerializeField]
    private List<ResourceData> ResourcesToSpawn;
    ...
    public void AddResourceToSpawn(ResourceData resource) {
        ResourceData existingResource = ResourcesToSpawn.Find(r => r.ResourceType == resource.ResourceType);
    ...
    private void SpawnResourceAround() {
        Vector2Int center2Int = new((int)transform.position.x, (int)transform.position.z);
        int checkedN = 0;
        foreach (ResourceData resource in ResourcesToSpawn) {
            ...
            while (remainingAmount > 0) {
                ...
                if (_data.WorldResourcesData.ResourcesOnScene.TryGetValue(targetCell, out ResourceView existingResource)) {
